...
int32_t libvibeModel_Sequential_Segmentation_8u_C1R(
    ...
    uint8_t matchingNumber = model->matchingNumber;
    uint8_t matchingThreshold = model->matchingThreshold;
    ...
    first_history(width * height, matchingThreshold, matchingNumber-1, historyImage, image_data, segmentation_map);

    /*for (int i = 0; i < 20; i++) {
        printf("%d ", segmentation_map[i]);
    }*/
    ...
    for (int i = 1; i < NUMBER_OF_HISTORY_IMAGES; ++i) {
        ...
        next_history(width * height, matchingThreshold, matchingNumber-1, pels, image_data, segmentation_map);
    ...
    uint8_t* mask = segmentation_map;
    output_mask(width * height, mask, COLOR_FOREGROUND, -1);
